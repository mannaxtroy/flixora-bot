...
# -*- coding: utf-8 -*-
"""
Flixora MovieBox - single file, polling, fast
"""
...
async def search_tpb(query):
    """Fast search on ThePirateBay mirror."""
    results = []
    try:
...
async def search_ddg(query):
    """Fallback: DuckDuckGo direct download search."""
    results = []
    try:
        async with httpx.AsyncClient(headers=HEADERS, timeout=12, follow_redirects=True) as client:
...
async def global_search(query):
    """Run all searches in parallel, merge and sort by seeds."""
    tasks = [
        search_tpb(query),
        search_nyaa(query),
        search_yts(query),
